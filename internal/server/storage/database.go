...
func (r *PGMetricRepository) Get(key string, mtype string) (metrics.Metric, error) {
	if !metrics.IsValidMetricType(mtype) {
		return metrics.Metric{}, errtypes.MakeServerError(errors.New(fmt.Sprintf("invalid metric type '%v'", mtype)))
	}
	...
	work := func() error {
		...
		if err := res.Scan(&value, &delta); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return errtypes.MakeNotFoundError(errors.New(fmt.Sprintf("metric '%v' of type '%v' not found", key, mtype)))
			}
